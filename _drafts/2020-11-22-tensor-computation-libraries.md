...
1. How is the computational graph is represented and built?
   - PyTorch dynamically builds (and eagerly evaluates) an explicit computational graph.
   - For more on how PyTorch builds computational graphs, see
     [`jdhao`’s introductory blog post on computational graphs in
     PyTorch](https://jdhao.github.io/2017/11/12/pytorch-computation-graph/), or [the PyTorch docs
     on autograd mechanics](https://pytorch.org/docs/stable/notes/autograd.html),
...
1. What is the computational graph used for?
   ...
   - Theano is often remembered as a library for deep learning research, but it’s so much more than
     that!
1. How does the library ensure "best execution" for computation?
   - The [original (and now sadly discontinued) Theano](https://github.com/Theano/Theano) used the
     GCC C compiler for CPU computation, and the NVCC CUDA compiler for GPU computation.
   - The [Theano-PyMC fork](https://github.com/pymc-devs/Theano-PyMC) (soon to be renamed to Aesara)
     project [will use JAX as a
