...
2. Autoregressive
     ...
     * I'll explain this more later, but it's worth saying now: autoregressive
       ...
       to suggest that this is a worthwhile tradeoff.
...
3. Generative
     ...
     * More formally, a generative model models the joint distribution $$P(X,
       Y)$$ of the observation $$X$$ and the target $$Y$$. Contrast this to a
     ...
     * This is unnecessary word #1: any autoregressive model can be run
       ...
       ..., x_k$$ and predict $$x_{k+1}$$. Then use $$x_2, x_3, ..., x_{k+1}$$
       to predict $$x_{k+2}$$, and so on.
...
4. Sequence model
     ...
     * Although sequence models are designed for sequential data (duh), there
...
- [WaveNet by Google
  ...
  16-bit precision samples, which means a 65,536-way-softmax per time step!)
...
2. Autoregressive models are supervised.
...
3. Autoregressive models work on both continuous and discrete data.
...
4. Autoregressive models are very amenable to conditioning.
     ...
     * Google DeepMind followed up their original PixelRNN paper with [another
       paper](https://arxiv.org/abs/1606.05328) that describes one way to
       ...
       decoder with a conditional PixelCNN.
     * WaveNet goes even futher and employs "global" and "local" conditioning
       ...
       music genre, musical instrument, etc.

5. Generating output sequences of variable length is not straightforward.
...
6. Autoregressive models can model multiple timescales
