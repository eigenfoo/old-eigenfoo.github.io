...
Here's a short story you might know: you have a black box whose name is the
...
testing mode and throw in some more $$x_i$$s, for which you don't have the
corresponding $$y_i$$s. You hope the $$y_i$$s that come out are in some sense
the "right" ones.
...
_learning using privileged information_, or LUPI.
...
examples perfectly (or that it may not be desireable to do so), and introduce
so-called _slack variables_, $$ \xi_i \geq 0 $$, which measure the
misclassification of example $$i$$. With that, the optimization becomes:
...
Clearly, there's a lot of room for potential in the LUPI paradigm, as well as a
lot of reasons to be skeptical. I'm hoping to write follow up posts later.
