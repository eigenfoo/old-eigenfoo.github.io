...
excerpt: "What is LDA, why does it work, and how do I use it?"
header:
...
common dimensionality reduction and data-preprocessing technique for
classification problems. However, that's something of an understatement: it does
so much more than "just" dimensionality reduction.
...
sometimes considered a classification algorithm in itself.

The goal of this post is to give a _comprehensive_ introduction to, and
explanation of, LDA. As such, LDA is explained in three ways:
...
This is probably too much for one blog post, but my hope is that there's
...
Below is great illustration from [Sebastian
...
src="https://raw.githubusercontent.com/eigenfoo/eigenfoo.github.io/master/assets/images/lda-pic.png">
...
A standard result from probability is that $$ cov(A^T X) = A^T cov(X) A $$, where
$$A$$ is some constant matrix and $$X$$ is a random vector. From this, it
follows that 
...
Then, it's a simple matter to write $$J_1$$ in terms of $$A$$, and optimize! I
don't want to go into the algebra here, but if you really want to see it,
Fukunaga is a great resource.
...
implies that LDA is must reduce the dimension to at least $$n-1$$!
...
<img style="float: right; margin:20px 20px" width="300" height="300"
src="https://raw.githubusercontent.com/eigenfoo/eigenfoo.github.io/master/assets/images/lda-pic.png">
...
QDA is a generalization of LDA. LDA has a baked-in assumption that all classes
follow a Gaussian distribution with the same covariance matrix (this is made
more explicit in an [alternative derivation on
Wikipedia](https://en.wikipedia.org/wiki/Linear_discriminant_analysis#LDA_for_two_classes)).

QDA doesn't make this assumption of
[homoskedasticity](https://en.wikipedia.org/wiki/Homoscedasticity), and attempts
to estimate the covariance of all classes. While this might seem like a more
robust algorithm (Fewer assumptions! Occam's razor!), this means there is a much
larger number of parameters to estimate. In fact, the number of parameters grows
quadratically with the number of classes! So you can guarantee that your
covariance estimates are reliable, you might not want to use QDA.
...
> _categorical_ independent variables and a _continuous_ dependent variable,
> whereas discriminant analysis has _continuous_ independent variables and a
> _categorical_ dependent variable (i.e. the class label). 
